...
class PresentationConverter2:
    ...
    def convert_to_pdf(self):
        ...
        for i, (value1, value2) in enumerate(zip(data1, data2), start=1):
            ...
            self.add_text_to_slide(slide, str(value1.value), Pt(210), Pt(85), Pt(40), Pt(20))
            self.add_text_to_slide2(slide, str(value2.value), Pt(210), Pt(55), Pt(40), Pt(20), font_size=16)
    ...
    def create_gui(self):
        ...
        # Заголовок
        title_label = tk.Label(root, text="Генератор додатків до свідоцтв", font=("Arial", 20, "bold"),
                               highlightthickness=0, bg="#d39684")
        title_label.grid(row=0, column=0, columnspan=4, pady=10)

        # Картинка
        image3_path = os.path.join(base_path, "image3.png")

        if os.path.exists(image3_path):
            image3 = tk.PhotoImage(file=image3_path)
            image_label3 = tk.Label(root, image=image3)
            image_label3.grid(row=1, column=3, rowspan=6, padx=10)
...
def main():
    converter = PresentationConverter2()
